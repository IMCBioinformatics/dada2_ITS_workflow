...
This is a snakemake workflow for profiling fungal communities from amplicon sequencing data using dada2. DADA2 tutorial can be found from https://benjjneb.github.io/dada2/ITS_workflow.html.

<br>

In the dada2 pipeline, species assignment is accomplished through the application of a naive Bayesian classifier method (https://pubmed.ncbi.nlm.nih.gov/17586664/), where a strict requirement of a 100% nucleotide identity match between the reference sequences and the query is employed.
...
* Taxonomic assignment tables using RDP classifier and UNITE database.
...
```bash
git clone https://github.com/IMCBioinformatics/dada2_ITS_workflow.git
```
...
##### 4.2.2 output/taxonomy
   - UNITE_RDP.tsv
   - vsearch_dada2_merged.tsv (adding vsearch results and merging vsearch and dada2 UNITE taxonomy assignmnets)
...
##### 4.2.4 fasta_files
   - ASVs_id.fasta   (headers as asv_ids e.g.asv_1, asv_2, etc)
   - ASVs_seqs.fasta (headers as asv_ids plus taxonomy information)
   - ASVs_tax.fasta  (headers as asv sequences)
