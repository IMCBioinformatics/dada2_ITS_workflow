rule combining_annotations:
    input:
        file=expand(config["output_dir"] + "/taxonomy/dada2_tables/{ref}_RDP.tsv", ref=config['RDP_dbs'].keys()),
        seqs=rules.removeChimeras.output.rds 
    output:
        table=config["output_dir"]+"/taxonomy/dada2_tables/"+"annotation_combined_dada2.txt"
    conda:
        "dada2"
    script:
