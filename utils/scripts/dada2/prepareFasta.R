...
df_selected <- df %>%
  select(asv_id, asv_seq, matches("UNITE", ignore.case = TRUE)) 
...
df_selected <- df %>%
  select(asv_id, asv_seq, matches("UNITE", ignore.case = TRUE)) %>% 
  mutate(taxonomy = paste0(asv_id,";",apply(.[, 3:9], 1, format_taxonomy)))


# Write the data to a FASTA file with asv ids
write_fasta(df_selected %>% select("asv_id","asv_seq"), snakemake@output[['id_fas']])
