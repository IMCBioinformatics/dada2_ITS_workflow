---
title: "From ITS Amplicon Raw Data to ASVs (Amplicon Sequence Variants)"
...
The overall read quality plots offer a comprehensive assessment of the sequencing run's data integrity, highlighting raw and processed read quality. The plots display quality scores (Y-axis) against read positions (X-axis), showing a
typical trend where quality decreases towards the end of the reads. The left panel represents forward reads quality and The right panel illustrates reverse reads quality aggregated for all samples. In gray-scale is a heat map of the frequency of each quality
score at each base position is shown. The mean quality score at each position is shown by the green line, and the quartiles of the quality score distribution by the orange lines. The red line shows the scaled proportion of reads that extend to at least that
position (this is more useful for other sequencing technologies, as Illumina reads are typically all the same length, hence a flat red line will be expected).
...
Read length distribution is shown in this plot for all reads in all samples after each step. This shows how removing primer (if applicable), quality trimming, and error rate filtering affects the length 
of reads during the process. The Y-axis represents the abundance of reads stacked per sample, while the X-axis shows the read lengths in base pairs (bp). Please note that the plot is initially zoomed in on the last 50 base pairs of the read length. To view the full distribution of read lengths, especially if they have become shorter, you will need to zoom out.
...
p <- ggplot(final_data %>% filter(num_reads > 0 & readtype=="R1"), aes(x = read_length, y = num_reads, fill = sample_name)) +
  ...
  xlab(NULL) +
  ylab(NULL) +
  xlim(0,max(final_data$read_length)+15) +
...
default_x_range <- c( max(final_data$read_length)-80,  max(final_data$read_length)+30)  # Adjust these values to your desired default zoom range for the x-axis
...
plotly_p <- plotly_p %>% plotly::ggplotly() %>%
  layout(
    xaxis = list(
      ...
      rangemode = "normal",
      range = default_x_range  # Set the default zoom range
    ),
    xaxis2 = list(
      rangeslider = list(visible = TRUE),
      rangemode = "normal",
...
This plot provides a step-by-step overview of the changes in read counts throughout the DADA2 pipeline for all samples. It illustrates how many sequencing reads are discarded at each stage of the processing workflow, from the initial raw data to the
final remained cleaned data. Each step, including quality trimming, filtering, denoising, merging, and chimera removal, is shown with the corresponding number of reads lost. The final bar indicates the total number of high-quality reads remaining after all
processing steps.
...
This plot compares the distribution of read counts at the beginning and end of the data processing pipeline for all samples. The left panel shows the initial distribution of raw reads and the right panel illustrates the final distribution of clean reads
after all the steps of trimming, filtering, denoising, merging, and chimera removal have been applied. Each bar represents the number of samples falling within specific read count ranges.
...
## Fungal composition in the positive controls

<br>

The original composition of the Zymo control includes a balanced mix of 10 bacterial species and 2 yeast species. When amplifying the ITS region, the expected outcome is that only the yeast species, Saccharomyces cerevisiae and Cryptococcus neoformans, should be amplified.

<br>

```{r positive control, warning=F, message=F, results='asis',echo=F,fig.width=10, fig.height=7}
...
if (length(samples) > 0 && !all(samples == "")) {
  ...
  # Define known Fungal community counts for each reference file
  ...
  for (i in samples) {
    ...
    for (j in names(ref_files)) {
      ...
      # Retrieve the expected Fungal community counts for the current reference file
      expected_percentage <- community_percentage[[j]]
      
      match.ref <- as.data.frame(sapply(colnames(unqs.mock), function(x) grepl(x, mock.ref)))
      
      match.ref$species <- stringr::str_split_fixed(names(mock.ref), "_18S|_Mitochondria|_mitochondria|_ITS", 2)[, 1]
      
      maps <- gather(data = match.ref, key = "seqs", value = "present", -species) %>%
        dplyr::filter(present == TRUE)

        #sometimes there is overlap between positive control reference files Fungal composition so samples get matched with a reference when there is >60% similar Fungal composition
        ...
        temp<-left_join(expected_percentage,temp,by="Taxa") %>%
        mutate(Observed = replace_na(Observed, 0))


        original<-data.frame(Taxa = c("Pseudomonas_aeruginosa", "Escherichia_coli",
                    ...
                    Original = c(12,12,12,12,12,12,12,12,2,2))

        temp<-left_join(original,temp,by="Taxa") %>%
        ...
        A <- ggplot(reshape2::melt(as.data.frame(temp)), aes(x = variable, y = value, fill = Taxa)) +
          ...
          xlab(NULL) +
          ggtitle(paste0("Fungal profile in ", i)) +
          theme(legend.position = "right",
        ...
        B <- kable(temp, format = "html", caption = "Percentage table of taxa found in the positive control sample") %>%
          kable_styling() %>%
...
<br>
This graph depicts the prevalence and abundance of Amplicon Sequence Variants (ASVs) across multiple samples. The X-axis represents the abundance of each ASV, which is the total count of sequences assigned to that ASV. The Y-axis represents the prevalence of
each ASV, indicating the number of samples in which the ASV is found. Each point on the graph represents a single ASV, with its position determined by its abundance and prevalence. Points higher on the Y-axis are found in more samples, indicating higher
...
files <- list.files(path = snakemake@params[["length_distribution"]], pattern = "Sequence_Length_distribution", full.names =TRUE)
...
If no sample profile is displayed, try selecting different samples from the list. To view specific sample profiles in the Krona plot, you need to click on the samples listed on the left. You can also use 
the search box at the top to find specific taxa. Additionally, the Krona plot provides options to adjust the taxonomy depth, font size, and chart size to customize your view. Use the '+' and '-' buttons 
next to each option to make these adjustments. If needed, you can also take a snapshot of your current view for reference or sharing.
